...
module.exports = themeSettings = () => {
  const settingsThemeTab = createElementFromHTML(
    `<div class="betterteams-mod-setting-tab"><h2 class="app-font-base-bold">Themes</h2></div>`
  );
  ...
  themeButton.innerHTML =
    cog_icon +
    '<span class="single-line-truncation" title="Themes">Themes</span>';
  themeButton.addEventListener("click", (e) => {
    if (e.currentTarget.className.toString().includes("selected")) return;
    document.getElementsByClassName(
      "options-settings-left-item ng-scope selected"
    )[0].className = "options-settings-left-item ng-scope";
  ...
  for (let i = 0; i < origButtons.length; i++) {
    origButtons[i].addEventListener("click", (e) => {
      if (!origButtons[i].className.includes("selected"))
        origButtons[i].className += " selected";
      themeButton.className =
        "options-settings-left-item betterteams-settings-left-item";
      const rightTabs = document.getElementsByClassName(
        "options-settings-right-item ng-scope"
      ...
      const customSettingTabs = document.getElementsByClassName(
        "betterteams-mod-setting-tab"
      );
      for (let i = 0; i < customSettingTabs.length; i++) {
        customSettingTabs[i].remove();
      }
    });
  }
  ...
  document
    ...
    .children[0].appendChild(themeButton);
  reloadThemeTab(settingsThemeTab);
};

const reloadThemeTab = (settingsThemeTab) => {
  for (const [_, theme] of global.tm.themes.entries()) {
    ...
    div.innerHTML = `
        <div class="betterteams-mod-status ${
          theme.isEnabled
            ? "betterteams-mod-enabled"
            : "betterteams-mod-disabled"
        }"></div>
        <span class="betterteams-mod-name">${theme.name}</span>
        <span class="betterteams-mod-author">${theme.author}</span>
        <span class="betterteams-mod-description">${theme.description}</span>
        <a class="betterteams-mod-url" target="_blank" href="${theme.url}">${
      theme.url
    }</a>
      `;
    div.children[0].addEventListener("click", (e) => {
      if (theme.isEnabled) {
        global.tm.disableTheme(theme.id);
      } else {
        global.tm.enableTheme(theme.id);
      }
      e.currentTarget.className = `betterteams-mod-status ${
        theme.isEnabled ? "betterteams-mod-enabled" : "betterteams-mod-disabled"
      }`;
    });
    settingsThemeTab.appendChild(div);
  }
};
